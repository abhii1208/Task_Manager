...
export const TaskList = ({ tasks, meta, onPageChange, onEdit, onDelete }: TaskListProps) => {
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-hidden rounded-2xl border border-violet-border bg-white shadow-soft">
        <div className="hidden grid-cols-[1.5fr_1fr_1fr_1fr_120px] gap-3 border-b border-violet-border bg-brand-soft-bg px-4 py-3 text-xs font-semibold uppercase tracking-wide text-text-muted md:grid">
          <p>Task</p>
          <p>Stage</p>
          <p>Priority</p>
          <p>Due Date</p>
          ...
          {tasks.map((task) => (
            <div key={task.id} className="grid gap-3 px-4 py-4 md:grid-cols-[1.5fr_1fr_1fr_1fr_120px] md:items-center">
                ...
                {task.tags.length ? (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {task.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="rounded-full bg-brand-soft-bg px-2 py-0.5 text-xs font-medium text-violet-700">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">{formatDate(task.dueDate)}</p>
              </div>
              ...
              <div className="flex justify-start gap-1 md:justify-end">
                ...
                <Button variant="ghost" className="h-8 w-8 px-0 text-danger hover:bg-red-50" onClick={() => onDelete(task)} aria-label="Delete task">
                  <Trash2 size={14} />
                </Button>
              </div>
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-violet-border bg-white p-3 shadow-soft">
